...
{
	$viewId=$_GET["rid"];
	$delQry="update  tbl_company set company_status='2' where company_id  =$viewId";
    if($con -> query($delQry))
	{
		?>
        <script>
...
<?php
$selquery="select * from  tbl_company s inner join tbl_place p on s.place_id=p.place_id inner join tbl_district d on p.district_id=d.district_id where company_status='0' ";
...
while($data = $result -> fetch_assoc())
{
 ...
 <td><a href="NewCompany.php?rid=<?php echo $data["company_id"]?>">Reject</a></td>
    </tr> 
